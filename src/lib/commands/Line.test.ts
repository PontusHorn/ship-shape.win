...
describe('Line', () => {
	describe('toString', () => {
		it('should return one-liner format for short coordinates with "to" method', () => {
			const line = new Line(new Position(px(10), px(20)), 'to');

			expect(line.toString()).toBe('line to 10px 20px');

			line.method = 'by';

			expect(line.toString()).toBe('line by 10px 20px');
		});

		it('should return multi-line format when one-liner exceeds 80 characters', () => {
			// Create a long coordinate string using raw values
			const longX = raw('calc(100vw - 50px + 10em - 5rem + 20ch)');
			const longY = raw('calc(100vh - 30px + 15em - 8rem + 25ch)');
